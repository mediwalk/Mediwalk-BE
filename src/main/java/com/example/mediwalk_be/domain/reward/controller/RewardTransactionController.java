...
public class RewardTransactionController {
	...
	public ResponseEntity<RewardTransactionResponse> create(
			@AuthenticationPrincipal AuthenticatedUser currentUser,
			@RequestBody CreateRewardTransactionRequest request) {
		return ResponseEntity.status(HttpStatus.CREATED)
				.body(RewardTransactionResponse.from(rewardTransactionService.create(currentUser.userId(), request)));
	}
	...
	@GetMapping
	@Operation(summary = "리워드 거래 목록 조회", description = "현재 사용자의 리워드 거래 목록을 정렬 기준(sort=latest|oldest)으로 조회합니다. startDateTime/endDateTime을 함께 전달하면 기간 필터링됩니다. 적립 건은 eventTitle·locationName·eventType으로 표기하고, accumulationCompleted는 폐의약품 수거(MEDICINE_COLLECTION) 적립 완료 뱃지용입니다.")
	public List<RewardTransactionResponse> findByUserId(
		...
		return rewardTransactionService
				.findByUserIdWithOptionalPeriod(currentUser.userId(), startDateTime, endDateTime, sort, PageRequest.of(page, size)).stream()
				.map(RewardTransactionResponse::from)
				.toList();
	}

	@GetMapping("/medicine-collections")
	@Operation(summary = "폐의약품 수거 적립 목록 조회", description = "현재 사용자의 폐의약품 수거(MEDICINE_COLLECTION) 적립 내역만 최신순/오래된순으로 조회합니다. startDateTime/endDateTime을 함께 전달하면 이번 달 등 기간별 상세 리스트에도 사용할 수 있습니다.")
	public List<RewardTransactionResponse> findMedicineCollectionTransactions(
			...
			@RequestParam(defaultValue = "20") int size) {
		return rewardTransactionService
				.findMedicineCollectionTransactions(currentUser.userId(), startDateTime, endDateTime, sort, PageRequest.of(page, size)).stream()
				.map(RewardTransactionResponse::from)
				.toList();
	}

	@GetMapping("/count")
	@Operation(summary = "폐의약품 수거 건수 조회", description = "현재 사용자의 폐의약품 수거 적립 건수를 조회합니다. startDateTime/endDateTime을 함께 전달하면 기간 건수(월별 등)를, 미전달 시 누적 건수를 반환합니다.")
	public RewardTransactionCountResponse countByUserId(
			@AuthenticationPrincipal AuthenticatedUser currentUser,
			@RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDateTime,
			@RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDateTime) {
		long totalCount = rewardTransactionService.countByUserIdWithOptionalPeriod(currentUser.userId(), startDateTime, endDateTime);
		return new RewardTransactionCountResponse(currentUser.userId(), totalCount);
	}
